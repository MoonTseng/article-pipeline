...
2. **正文**：1500-2500字，这是硬性要求，低于1500字不合格
   - 开头用一两句话直击要害，抛出核心信息
   - 中间展开：技术原理讲清楚、行业背景补充到位、上下游影响分析透彻
   - 适当引用数据、对比竞品、举具体案例（不要空谈）
   - 结尾加入犀利点评/预判/给读者的思考，不要烂尾
   - 每个小节用 ## 二级标题分隔，提升可读性
3. **标签**：3-5个中文标签
...
### 表格：有数据对比时必须用表格
适合场景：性能对比、参数对比、价格对比、功能对比、时间线
示例：
| 模型 | 参数量 | 推理速度 | 价格 |
|------|--------|----------|------|
| GPT-4 | 1.8T | 40 tok/s | $30/M |

### 引用块：用于强调金句、关键数据、专家观点
示例：
> 📊 核心数据：Claude 3.5 Sonnet 在 SWE-bench 上达到 49%，比 GPT-4 高出 12 个百分点。

### 图片占位：每篇文章插入 2-3 个图片占位符
用 `![描述](img:关键词)` 格式，关键词用于后续自动配图
示例：
![AI芯片性能对比图](img:ai-chip-benchmark)
![手机改造服务器示意图](img:phone-server-diy)

### 排版节奏规则
- 每 300-400 字插入一个视觉元素（表格/引用块/图片占位）
- 不要连续超过 4 段纯文字，中间穿插视觉元素打断
- 关键数据用 **加粗** 或 `代码块` 突出
- 列表（有序/无序）比大段文字更易读
